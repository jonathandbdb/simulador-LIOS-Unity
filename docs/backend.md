...
- **`/admin/provisioning` (QR de Android Enterprise, recuperación remota de tablets)**: si una
  ...
  **Hoja imprimible "de caja"**: botón `prov.print_button` ("Imprimir") junto al QR en pantalla,
  `onclick="window.print()"` — sin JS extra ni dependencias nuevas. La página tiene un
  `<div class="print-sheet">` (oculto en pantalla, `provisioning.html`) con SOLO el QR grande y
  los 5 pasos del CLIENTE en español (`prov.print_step1..5`, texto distinto de `prov.instructions`
  que es para el operador): encender hasta bienvenida, 6 taps, escanear, esperar sin tocar nada
  (WiFi si lo pide), no agregar cuenta Google. Un `@media print` inline en la plantilla hace
  `visibility: hidden` a todo `body *` salvo `.print-sheet` (patrón estándar, sin tocar
  `admin.css` ni `base.html` — queda contenido a esta página). Si el QR se generó con WiFi
  (`wifi_ssid` en el contexto), la hoja muestra `prov.print_wifi_note` + el SSID en texto, **nunca
  la password** (el QR la lleva igual adentro; el JSON completo con la password solo se ve en la
  card `prov.json_title`, ya existente, para el operador). **"Descargar PNG" evaluado y
  descartado**: aunque `segno` (ya dependencia) puede serializar PNG, servirlo requeriría que el
  botón dispare OTRA request al backend con los mismos campos de WiFi (para que el PNG coincida
  con el QR ya generado) — mismo endpoint `POST /admin/provisioning` pero ramificando el tipo de
  respuesta (HTML vs `image/png` con `Content-Disposition`), lo que ya no es trivial. El SVG
  inline + "Imprimir"/"Guardar como PDF" del propio navegador cubre el caso de uso (hoja para la
  caja) sin ese costo.
...
- **Marca visible `IOLSIMULATOR` (2026-09-03)** → renombre comercial (material del congreso: "IOL" en negrita + "SIMULATOR" fina, en texto plano `IOLSIMULATOR`, una sola palabra, sin traducir). Tocó `FastAPI(title=...)` y la respuesta de `GET /` (`app/main.py`) y `"app.title"` en `app/admin/i18n.py` (es/en, es marca así que no cambia por idioma) — es todo lo que era marca visible para humanos en el backend. Los identificadores técnicos **conservan** `simulador`/`com.simulador.*` sin cambios: paquetes Android, `s3_bucket="simulador-updates"`, `database_url` (`simulador:changeme@db/simulador`), rutas de archivo (`apk/{app}/simulador-{app}-{version}.apk`), el device dummy del seed y el propio nombre del repo — es deuda de identificadores, no de marca, y no está en el alcance de este cambio.
...
- **Agregar una variable nueva a `Settings` (`config.py`) NO alcanza — también hay que agregarla al bloque `environment:` del servicio `api` en `docker-compose.yml`, o la app nunca la ve aunque esté en `.env`.** `Settings` (pydantic-settings) lee del proceso del contenedor, no del `.env` del host directamente; `docker-compose.yml` es el que decide qué variables de `.env` (vía interpolación `${VAR}`) se inyectan como `environment:` de cada servicio — una variable ausente de ese bloque queda invisible para la app aunque figure en `.env.example`/`.env`. Bug real encontrado y corregido (2026-09-24): `PROVISIONING_SIGNATURE_CHECKSUM` estaba en `config.py` y `.env.example` desde que se implementó el QR de provisioning, pero faltaba en el `environment:` de `api` — en prod, `env | grep PROVISIONING` daba vacío dentro del contenedor pese a estar seteada en el `.env` del server, así que `/admin/provisioning` caía siempre al checksum de paquete (el que caduca con cada release) sin que nadie lo notara. Verificado en vivo tras el fix: `docker compose config` muestra `PROVISIONING_SIGNATURE_CHECKSUM` resuelta en el servicio `api`, y `docker compose exec api env | grep PROVISIONING` la confirma dentro del contenedor con el valor de `.env`. Al agregar una `Settings` nueva que deba ser configurable por `.env`, sumarla también acá — no asumir que "está en `.env.example`" es suficiente.
