...
| `Assets/Plugins/Android/com/simulador/kiosk/SimuladorDeviceAdminReceiver.java` (nuevo, Fase A) | Subclase de `DeviceAdminReceiver`, componente admin activo del Device Owner (`adb shell dpm set-device-owner com.simulador.tablet/com.simulador.kiosk.SimuladorDeviceAdminReceiver`, ver `scripts/provision-tablet.sh`). `onProfileProvisioningComplete` lanza la Activity principal del propio paquete al terminar el flujo de QR/Android Enterprise provisioning (ver fila siguiente y `docs/builds-deploy.md` "Recuperación remota por QR") — camino que la Fase A por cable NO dispara. Se compila también en el visor (Unity compila todo `.java` suelto bajo `Plugins/Android/` en ambos targets) pero queda INERTE ahí: su manifest nunca declara el `<receiver>` (lo inyecta `TabletManifestPatcher.cs` solo durante el build de tablet, ver `docs/builds-deploy.md`). `onEnabled`/`onProfileProvisioningComplete` mandan telemetría (`prov_admin_enabled`/`prov_complete`, ver fila `ProvisioningTelemetry.java` más abajo). |
| `Assets/Plugins/Android/com/simulador/kiosk/ProvisioningModeActivity.java` y `PolicyComplianceActivity.java` (nuevo, Fase C QR provisioning) | Las dos Activities que Android 12+ exige declarar para el flujo unificado de QR/Android Enterprise provisioning — sin ellas el asistente corta con "No se puede configurar el dispositivo" (confirmado en campo con el QR real, PHILCO Android 13 y Lenovo Android 16). `ProvisioningModeActivity` responde `android.app.action.GET_PROVISIONING_MODE` con `PROVISIONING_MODE_FULLY_MANAGED_DEVICE` (única modalidad que soporta el proyecto). `PolicyComplianceActivity` responde `android.app.action.ADMIN_POLICY_COMPLIANCE` con `RESULT_OK` sin mostrar nada NI relanzar la app — `SimuladorDeviceAdminReceiver.onProfileProvisioningComplete` (fila de arriba) ya lo hace después; relanzarla desde acá también duplicaría el arranque. Ninguna de las dos tiene UI (ambas se cierran solas en `onCreate`). Declaradas por `TabletManifestPatcher.cs` (ver `docs/builds-deploy.md`), INERTES en el visor por el mismo patrón que `SimuladorDeviceAdminReceiver`. `onCreate` de ambas manda telemetría (`prov_get_mode`/`prov_policy_compliance`, ver fila siguiente). |
| `Assets/Plugins/Android/com/simulador/kiosk/ProvisioningTelemetry.java` (nuevo, diagnóstico QR) | Java puro sin dependencias: `send(Context, BaseBundle adminExtras, String event, String detail)` manda un POST best-effort a `{backend_url}/api/log` (mismo contrato que `UpdateManager`/`LicenseManager`, `HttpURLConnection` + JSON armado a mano, thread aparte con `join(4500ms)` para que el POST salga antes de que el proceso muera) desde cada etapa Java del provisioning por QR — sin adb del lado del cliente durante el asistente, es la única forma de ver hasta dónde llegó una provisión que se cae con "Something went wrong". `device_id` = `"prov-" + Settings.Secure.ANDROID_ID`. `extractAdminExtras(Intent)` centraliza el cast null-safe de `EXTRA_PROVISIONING_ADMIN_EXTRAS_BUNDLE` (PersistableBundle) que usan las 4 etapas llamantes; `resolveBackendUrl` lee la key `"backend_url"` del admin extras bundle del QR, con fallback a `https://vr.conecta.sh` (`// SIM: atajo deliberado`, ver `docs/builds-deploy.md` "Recuperación remota por QR" para el detalle de eventos/lectura en `/admin/logs`). Se compila también en el visor (mismo patrón `.java` suelto) pero solo se invoca desde las Activities/receiver de kiosco, INERTES ahí. |
...
| `Assets/Scripts/Editor/TabletManifestPatcher.cs` (extendido) | Además de lo que ya inyectaba (kiosco, updates), declara `<activity android:name="com.simulador.deck.DeckActivity">` SOLO en el build de tablet (`exported="false"`, `screenOrientation="landscape"` fijo porque el deck es 16:9, sin permisos nuevos) — el visor no la necesita. También declara `ProvisioningModeActivity` y `PolicyComplianceActivity` (fila de arriba): `exported="true"` + `permission="android.permission.BIND_DEVICE_ADMIN"` (mismo permiso que el `<receiver>` del Device Admin — las invoca el proceso del asistente de configuración, fuera de la app), `intent-filter` con su action + `category.DEFAULT`, tema `Theme.Translucent.NoTitleBar` (sin UI). |
